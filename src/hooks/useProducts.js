...
export function useProducts(canalSeleccionado) {
    ...
    const moduleProducts = useMemo(() => {
        ...
        const orderMap = {
            'Usuario Adicional (Micro a Especial II)': 1,
            ...
            'Establecimiento Adicional (Planes Ilimitados)': 6,
            'Establecimiento Adicional (Plan Contable)': 7,
            'Empresa adicional': 8,
            'Punto de venta': 9,
            'Soporte Técnico': 10,
            'Generación de ATS': 11,
            'Compras con ATS': 12,
            'Compras sin ATS': 13,
            'Módulo Documentos Recibidos': 14,
            'Factura Recurrente': 15,
            'Módulo de Cobranzas': 16,
            'Módulo de Gestión Comercial': 17,
            'Módulo Médico': 18,
        };
    ...
    const emissionPointProduct = useMemo(() => products.find(p => p.name?.includes('Establecimiento Adicional') || p.name === 'Punto de venta'), [products]);
